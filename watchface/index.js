import { createWidget, widget, align, text_style, prop, show_level } from '@zos/ui'
...
WatchFace({
  ...
  buildDial() {
    ...
    // Re-render on wake; retry GPS if we never got a fix.
    createWidget(widget.WIDGET_DELEGATE, {
      resume_call: () => {
        this.render()
        if (!this.gotFix) this.acquireLongitude()
      },
      pause_call: () => {},
    })
  },
